...
class RSACrypt
{
    ...
    public function __construct($keyPath = null, $bits = 2048)
    {
        ...
        if (file_exists($this->privateKeyPath) && file_exists($this->privateKeyPath)) {
            if (file_get_contents($this->publicKeyPath) && file_get_contents($this->privateKeyPath)) {
                return;
            }
        }
        ...
        $config = array(
            ...
            'config' => public_path('Rsa/openssl.cnf')
        );
        ...
        openssl_pkey_export($resource, $this->privateKey, null, $config);
